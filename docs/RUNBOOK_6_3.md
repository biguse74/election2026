...
- [ ] **fetch_articles cron 시프트 (선택)**: KST 09:00·15:00·21:00 → 09:02·15:02·21:02로 옮기면 라이브 폴링과 충돌 없음
...
- [ ] **05~10분 안에** 라이브 화면 새로고침해서 첫 투표율 폴링값 표시되는지 확인
...
- [ ] 만약 18:30까지 데이터 0이면: phase가 **"개표 데이터 수신 대기"** (노랑) 유지 — OpenAPI 갱신 늦은 것. 자동 폴링이 계속 시도하므로 기다림
...
- [ ] 안 됐으면 phase는 **"개표 진행 중 · 정식 결과 대기"** (노랑) 유지 — 30분 보조 폴링이 6/4 06:00~11:30 동안 도는 중
...
**대응**:
1. Actions 로그 확인 — 폴링이 도는데 push가 reject 되는지
2. push 충돌이면 다음 cron이 rebase 후 재시도 — 1~2회 폴링 지연 정상
3. 5회 이상 실패면 수동 트리거
...
3. 시간 여유 있으면 스크래퍼 작성. 없으면 OpenAPI 폴링만 유지
...
2. 라이브 화면이 그 후 1회 폴링했는지 (자동 새로고침 60초마다)
