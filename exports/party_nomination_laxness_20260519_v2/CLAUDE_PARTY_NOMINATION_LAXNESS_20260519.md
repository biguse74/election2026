...
- 2022년 8회 지선 후보자 스냅샷은 현재 로컬 `data/candidates` 아래에서 확인되지 않음: 없음.
...
- 1억원 이상 최근 5년 체납은 양당 기준 17명, 전체 활성 후보 기준 24명으로 확인된다. “20명”은 기준이 다르거나 보정 전 수치일 수 있어 원문에는 재검증 필요로 둔다.
...
| B-2. 1억 이상 체납 | 양당 17명, 전체 후보 24명 확인. 20명 기준 재검증 필요 | 그래픽 2~3장 |
...
- 1억원 이상 체납 “20명”은 이 패키지 기준과 맞지 않는다. 양당 17명, 전체 활성 후보 24명으로 확인되므로, 20명은 재검증 필요로 둔다.
```
